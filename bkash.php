...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ){
     ...
     */
    add_filter('woocommerce_payment_gateways', 'neety_bkash_payment_gateways');
    function neety_bkash_payment_gateways( $gateways ){
        $gateways[] = 'neetyIT_Bkash';
        return $gateways;
    }
     ...
     * bKash gateway init
     */
    add_action('plugins_loaded', 'neety_bkash_plugin_activation');
    function neety_bkash_plugin_activation(){

        class neetyIT_Bkash extends WC_Payment_Gateway {
            ...
            public function __construct(){
                ...
                $this->neety_bkash_options_fields();
                $this->init_settings();
                ...
                add_filter( 'woocommerce_thankyou_order_received_text', array( $this, 'neety_bkash_thankyou_page' ) );
                add_action( 'woocommerce_email_before_order_table', array( $this, 'neety_bkash_email_instructions' ), 10, 3 );
            }


            public function neety_bkash_options_fields(){
            ...
            public function neety_bkash_thankyou_page() {
            ...
            public function neety_bkash_email_instructions( $order, $sent_to_admin, $plain_text = false ) {
                if( $order->get_payment_method() != $this->id )
                    return;
     ...
     */
    add_filter( "plugin_action_links_". plugin_basename(__FILE__), 'neety_bkash_settings_link' );
    function neety_bkash_settings_link( $links ) {

        $settings_links = array();
        $settings_links[] ='<a href="https://www.facebook.com/neetyitinstitute/" target="_blank">' . esc_html__( 'Follow US', 'stb' ) . '</a>';
        $settings_links[] ='<a href="' . admin_url( 'admin.php?page=wc-settings&tab=checkout&section=neety_bkash' ) . '">' . esc_html__( 'Settings', 'stb' ) . '</a>';
     ...
     */
    $bkash_charge = get_option( 'woocommerce_neety_bkash_settings' );
    if( isset($bkash_charge['bkash_charge']) ){
        if( $bkash_charge['bkash_charge'] == 'yes' ){


            add_action( 'woocommerce_cart_calculate_fees', 'neety_bkash_charge' );
            function neety_bkash_charge(){
                ...
                if( $current_gateway!='' ){
                    ...
                    if ( $current_gateway_id =='neety_bkash' ) {
                        $percentage = 0.0185;
     ...
     */
    add_action( 'woocommerce_checkout_process', 'neety_bkash_payment_process' );
    function neety_bkash_payment_process(){

        if($_POST['payment_method'] != 'neety_bkash')
            return;
     ...
     */
    add_action( 'woocommerce_checkout_update_order_meta', 'neety_bkash_additional_fields_update' );
    function neety_bkash_additional_fields_update( $order_id ){
     ...
     */
    add_action('woocommerce_admin_order_data_after_billing_address', 'neety_bkash_admin_order_data' );
    function neety_bkash_admin_order_data( $order ){

        if( $order->get_payment_method() != 'neety_bkash' )
            return;
     ...
     */
    add_action('woocommerce_order_details_after_customer_details', 'neety_bkash_additional_info_order_review_fields' );
    function neety_bkash_additional_info_order_review_fields( $order ){
     ...
     */
    add_filter( 'manage_edit-shop_order_columns', 'neety_bkash_admin_new_column' );
    function neety_bkash_admin_new_column($columns){
     ...
     */
    add_action( 'manage_shop_order_posts_custom_column', 'neety_bkash_admin_column_value', 2 );
    function neety_bkash_admin_column_value($column){
     ...
     * Admin Notice
     */
    add_action( 'admin_notices', 'neety_bkash_admin_notice__error' );
    function neety_bkash_admin_notice__error() {
        ?>
     ...
     * Deactivate Plugin
     */
    add_action( 'admin_init', 'neety_bkash_deactivate' );
    function neety_bkash_deactivate() {
